...
export default function StudentMessagePage() {
  ...
  const fetchConversations = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/conversations/principal");
  ...
  const sendMessage = async () => {
    ...
    try {
      await axios.post(
        `/api/conversations/principal/messages?conversationId=${selectedConversation._id}`,
        {
      ...
      setSelectedConversation({
        ...
        messages: [
          ...
          { content: messageInput, sender: { _id: user, model: "Principal" } },
        ],
      });
